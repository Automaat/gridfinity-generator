<script lang="ts">
	...
	const ERROR_TEXT_KEYS: Record<WorkerErrorCode, 'invalidParams' | 'wasmError' | 'outOfMemory' | 'timeout' | 'unknownError'> = {
...
{#snippet languagePicker()}
	<label class="flex items-center gap-1.5 text-[11px] font-medium text-zinc-500">
		<span class="sr-only">{t.language}</span>
		<select
			bind:value={$language}
			aria-label={t.language}
			class="rounded-md border border-zinc-800 bg-zinc-950/40 px-2 py-1 text-xs text-zinc-300 outline-none transition hover:border-zinc-700 focus:border-blue-500"
		>
...
<div class="flex h-screen flex-col bg-zinc-950 text-zinc-100 md:flex-row">
	...
	<aside class="hidden w-80 shrink-0 overflow-y-auto border-r border-zinc-800 bg-zinc-900 md:block">
		<div class="sticky top-0 z-10 flex items-center gap-2.5 border-b border-zinc-800 bg-zinc-900/95 px-4 py-3.5 backdrop-blur">
			{@render logo()}
			<div>
				<h1 class="text-sm font-bold leading-tight">{t.appName}</h1>
				<p class="text-[11px] text-zinc-500">{t.tagline}</p>
			</div>
			<div class="ml-auto">{@render languagePicker()}</div>
		</div>
	...
	<main class="relative flex-1">
		<Viewer
			...
			{edges}
			{loading}
			error={buildError && { title: t[ERROR_TEXT_KEYS[buildError.code]], message: buildError.message }}
		/>
		...
		<div class="shrink-0 border-b border-zinc-800 bg-zinc-900">
			...
			<div class="flex items-center justify-between gap-2.5 px-4 py-3">
				<div class="flex items-center gap-2.5">
					{@render logo()}
					<h1 class="text-sm font-bold">{t.appName}</h1>
				</div>
				<div class="flex items-center gap-2">
					{@render languagePicker()}
				<button onclick={() => (controlsOpen = false)} class="rounded-lg p-1 text-zinc-400 transition hover:bg-zinc-800 hover:text-zinc-200" aria-label={t.closeControls}>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
