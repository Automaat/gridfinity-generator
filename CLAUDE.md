...
│   │   ├── hex-lattice.ts      # Pure flat-top hex-lattice math, shared by bins + Skadis (engine-free, tested)
...
- **Skadis box:** A third top-level mode (`AppMode = 'bin' | 'baseplate' | 'skadis'`, URL marker `m=sk`, `SkadisParams` store + `SK_CODECS` with non-colliding `sk*` keys). NOT a Gridfinity bin — a plain hollow box sized freely in mm with snap-in hooks for an IKEA Skadis pegboard (5×15mm slots, **40mm** pitch both axes, ~5mm board). Coordinate frame: box centered on X, back face at **Y=0** (flush to the board), body extends **+Y**, hooks protrude into **−Y** through the slots; bottom rests at Z=0. `SkadisParams.mountType` picks the back-wall attachment: `'hook'` (snap-in, default) or `'screw'` (M5 holes). **Hook = a conventional Skadis hook** (NOT the old loose wedge with a 0.9mm nub, which did not grip; and NOT a support-free drop-catch — that stuck out and looked odd): a horizontal **arm passes through the slot**, then a **lip drops down behind the board**, so the inside corner captures the board — the box back wall holds the front face, the lip's front (catch) face (`CATCH_FRONT_Y`, 0.4mm behind the board back) holds the back face. Install = slide the arms into the slots, lower the box so the lips drop behind the bridges below them. The arm underside + lip bottom are overhangs, so **print the hooks with supports** (the user accepted this over the worse-looking support-free geometry). Built as a Y-Z profile (`hookProfile`) extruded `HOOK_W` along X (`prismAlongX` / a `YZ`-plane sketch), NOT stacked boxes. The lip drops `LIP_DROP` (~8mm) below the row center, so `HOOK_REACH_BELOW` in the layout keeps the lowest row clear of the floor. **Screw mount** = a plain Ø5.5 M5 clearance hole bored straight through the back wall, **flush** (no boss/counterbore — a thin wall has no meat to recess a head, and a boss bump looks wrong); shaft exits a slot, washer+nut clamp behind the board; no hooks, nothing protrudes either side. Both attach to the same `planSkadis` grid. All hook + screw constants MUST match across both geometry paths (`skadis-manifold.ts` STL/preview + `skadis-occt.ts` STEP). `SkadisParams` width/height/depth are **interior** sizes — the printed envelope adds a wall each side and a floor below via `outerDims(p)` (the single inner→outer conversion both geometry paths and the layout consume). Hook placement (columns/rows on the 40mm grid, centered on the OUTER width, capped to what fits) lives in the engine-free `skadis-layout.ts` (the single tested source). Box top is always open; `openFront` lowers the front wall between the side walls down to `frontWallHeight`, `openSides` lowers the left+right walls down to `sideWallHeight` (both absolute Z; `frontWallCutZ`/`sideWallCutZ` in `skadis-layout.ts` floor to 5mm and cap at `outerH` so a too-tall value is a no-op cut). The front cut spans only the interior width and each side cut only the interior depth, so a closed neighbour keeps its shared corner full height. When BOTH front and sides are open the two **front** corner posts are bounded only by lowered walls — they'd stand alone, so they're dropped to `max(frontWallCutZ, sideWallCutZ)` (blend, no pole); the back corners belong to the full-height back wall and stay. `lightweightWalls` punches a hex lattice through **all four walls + the floor**. The hex math (constants + `hexPolygon` + `hexCells`) lives in the **shared engine-free `hex-lattice.ts`** — the single source for ALL hex lattices in the app (bin dividers in `manifold-bin.ts`/`gridfinity.ts` and these Skadis walls), so every variant stays identical. The hexes are **flat-top**: a flat-top hole on a vertical wall has ~30°-from-vertical upper edges + a short top bridge, so it prints without supports, vs a pointy-top's 60° overhang. `hexCells` tessellates flat-top (1.5R column pitch, √3R row pitch, alternate columns dropped half a row). The replicad paths build the same hexagon via `drawPolysides(HEX_RADIUS, 6)` (no rotation = flat-top). `wallHexCutters` takes an axis: `X` (side walls), `Y` (back/front), `Z` (floor). The **back wall is hexed only below the hook rows** — a solid mount band (`MOUNT_BAND` below the lowest hook) stays around the hooks; cut **before** the hooks are added so they weld onto that solid band. The **front + side panels are sized to the EXPOSED wall height** (`frontH/sideZ − t`, centered within it) — NOT the full interior height — so the lattice always keeps a solid band at the top and the (possibly lowered) wall **finishes with a clean straight top edge, never a hex sliced by the rim**. Panels too small for a hex stay solid.
